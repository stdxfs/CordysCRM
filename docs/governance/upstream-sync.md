...
4. 仅在同步 PR 中把 `governance/upstream-baseline.env` 的 `UPSTREAM_COMMIT` 更新为已审查的候选 SHA。治理门禁会以这个 SHA 而不是旧基线校验来源文件与 Migration；因此官方变更不会被误报为本地 Patch，但任何候选外的本地 Patch 仍会失败。
5. 运行治理门禁；通过后以 `upstream-sync -> develop` PR 合并。
6. 完成 Staging、E2E、升级与回滚证据后，按 release 流程进入 `main`。

更新 `governance/upstream-baseline.env` 是上游同步 PR 的受审计动作，必须同时更新修改登记和验证证据。它绝不能出现在 `feature/*`、`fix/*` 或普通业务 PR 中。
