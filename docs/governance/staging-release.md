...
单人开发修订：下列项目是正式部署的运行手册骨架，不是已经生效的完整自动发布门禁，也不阻止保存候选。独立本地容器可承担开发验证。当前开发测试数据可重建，但每次重建仍须确定具体数据库并先备份；本地和服务器分别操作，禁止清空共享卷。代码合并不自动部署。

官方改写历史 SQL 时分别验证新库初始化与旧库升级。新库通过不证明旧数据可升级；关闭校验或单独 repair 不能补执行旧 SQL 的新增内容。需要保留旧库时，应恢复历史 checksum 并用新的补偿 Migration 交付 DDL；无需保留业务数据时可选择重建开发库。
