...
- 新行业功能优先通过菜单、路由、动态 Tab、扩展槽或独立业务组件接入；不得将功能散落到官方客户、订单、合同等核心页面。
- 修改官方页面前必须在 OpenSpec 变更中说明扩展槽、动态菜单、插件组件或独立页面为何不可行，并登记 `UPSTREAM_MODIFICATIONS.md`。
- 不得以仅隐藏前端按钮替代后端权限校验；新增页面、菜单和操作必须接入统一权限资源。
- 不在业务 PR 中批量格式化、无关重构或顺手升级前端核心依赖。
